...
export default function HandlingPage() {
    ...
    const fetchUsers = async () => {
        try {                        
            ...
            console.error("Failed. fetch logged in users:", err);
            console.log("filing in fetching")
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const updateRole = async (id: string, newRole: string) => {
        try {
            ...
            if (res.ok) {
                ...
                console.log("the eror is fromb backend")
            }
        } catch (err) {
            console.error(err);
        }
    ...
    const removeUser = async (id: string) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                console.log("correct  ed")
            } else {
                alert("Failed to delete user");
                console.log("see bakcend")
            }
        } catch (err) {
            console.error(err);
        }
        ...
        <div className="w-full min-h-screen bg-gray-50 p-8">
            <div className="max-w-7xl mx-auto bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="px-8 py-6 border-b border-gray-200 bg-white">
                    <h1 className="text-2xl font-bold text-black">User Handling</h1>
                    <p className="text-black/70 mt-1 text-sm">Manage roles and access for active users in the system</p>
                </div>
